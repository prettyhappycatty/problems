...
#    print(current[0],current[1])
    ...
    for i in range(len(neighbor)):
        ...
        x = current[0] + nei[0]
        y = current[1] + nei[1]
        if x < 0 or y < 0 or x > H-1 or y > W-1 or S[x][y] == "#" or flg[x][y] == 1:
#            print("con")
            continue
...
#        print(x,y,i,F[x][y][i],">?",F[current[0]][current[1]][i],nei[0] == vec[0] and nei[1] == vec[1])
        for j in range(len(neighbor)):
            if i == j and  F[x][y][i] > F[current[0]][current[1]][i]:#方向が変わらない場合のコスト
                F[x][y][j] = F[current[0]][current[1]][i]
                flg[x][y] = 1
                Q.append((x,y))
            elif i != j and F[x][y][j] > F[current[0]][current[1]][i]+1: #方向が変わるけど最小値の場合のコスト更新
